using System.Linq;
using System.Threading;
using System.Threading.Tasks;
using FluentValidation;
using FluentValidation.Results;
using FluiTec.Datev.Models.Rows;

namespace FluiTec.Datev.Models.Validators
{
	/// <summary>   The address row validator. </summary>
	public class AddressRowValidator : AbstractValidator<AddressRow>, IValidator<IDatevRow>
	{
		/// <summary>   Default constructor. </summary>
		/// <param name="accountLength">    Length of the account. </param>
		public AddressRowValidator(int accountLength)
		{
			RuleFor(address => address.AccountNumber.ToString()).NotNull().Length(accountLength).NotEmpty();
			RuleFor(address => address.Salutation).Length(min: 0, max: 30);
			RuleFor(address => address.Name).Length(min: 0, max: 50);
			RuleFor(address => address.ShortName).Length(min: 0, max: 15);
			RuleFor(address => address.Name_Enterprise).Length(min: 0, max: 50);
			RuleFor(address => address.Name_Person).Length(min: 0, max: 30);
			RuleFor(address => address.FirstName_Person).Length(min: 0, max: 30);
			RuleFor(address => address.NameExtension1).Length(min: 0, max: 50);
			RuleFor(address => address.NameExtension2).Length(min: 0, max: 36);
			RuleFor(address => address.AddressType).GreaterThanOrEqualTo(valueToCompare: 0).LessThanOrEqualTo(valueToCompare: 3);

			var validPostalTypes = new[] {"STR", "PF", "GK"};
			RuleFor(address => address.PostalAddressType)
				.Must(type => type == null || validPostalTypes.Contains(type))
				.WithMessage(errorMessage: "PostalAddressType must be NULL or one of the predefined ones.");

			RuleFor(address => address.Street).Length(min: 0, max: 36);
			RuleFor(address => address.PostBox).Length(min: 0, max: 10);
		}

		/// <summary>   Validates the specified instance. </summary>
		/// <param name="instance"> The instance to validate. </param>
		/// <returns>   A ValidationResult object containing any validation failures. </returns>
		public ValidationResult Validate(IDatevRow instance)
		{
			return base.Validate(instance as AddressRow);
		}

		/// <summary>   Validate the specified instance asynchronously. </summary>
		/// <param name="instance">     The instance to validate. </param>
		/// <param name="cancellation"> The cancellation. </param>
		/// <returns>   A ValidationResult object containing any validation failures. </returns>
		public Task<ValidationResult> ValidateAsync(IDatevRow instance,
			CancellationToken cancellation = new CancellationToken())
		{
			return base.ValidateAsync(instance as AddressRow, cancellation);
		}
	}
}