...
namespace FluiTec.DatevSharp.Validation
{
	public class DatevHeaderValidator : AbstractValidator<DatevHeader>
	{
		public DatevHeaderValidator()
		{
			...
			When(header => header.DataCategory.Number == bookingNumber, () => RuleFor(header => header.BookingType).InclusiveBetween(1,2));
			...
			When(header => header.DataCategory.Number == bookingNumber, () =>
				...
				.WithMessage($"BillingIntention must be one of the following values: {string.Join(",", validIntentions.Select(i => i.ToString()))}."));
